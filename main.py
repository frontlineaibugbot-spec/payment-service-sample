...
def get_exchange_rate(from_currency: str, to_currency: str):
    ...
    if from_currency not in EXCHANGE_RATES:
        push_log("warn", f"Unsupported source currency: {from_currency}", {
            "from_currency": from_currency,
        ...
        raise HTTPException(status_code=400, detail={
            ...
            "message": f"unsupported source currency: {from_currency}",
            "supported_currencies": list(EXCHANGE_RATES.keys()),
        })
    
    # Validate target currency
    if to_currency not in EXCHANGE_RATES[from_currency]:
        push_log("warn", f"Unsupported target currency: {to_currency}", {
            "from_currency": from_currency,
            "to_currency": to_currency,
        ...
        raise HTTPException(status_code=400, detail={
            "error": "UnsupportedCurrency",
            "message": f"unsupported target currency: {to_currency} for {from_currency}",
            "supported_currencies": list(EXCHANGE_RATES[from_currency].keys()),
        })
